...
This document describes the plugin as the code stands at 5.0.4. Where the code and the previous documentation disagreed, the code won. Anything surprising is listed under "Known issues" at the end rather than glossed over.
...
Verified against the 5.0.4 source. None of these are hidden from users; the user guide says what it needs to.
...
8. Version strings disagree: `kPluginVersion` 5.0.4, MainQuest `Version` "5.04", `plugin.cpp` logs "v4.0 refactored", EXIF `Model`/`Software` say "PrintScreen V4", and the two uncompiled `.rc` files say 4.0.0 and 5.0.0.
